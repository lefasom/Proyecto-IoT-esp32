...
def wait_for_finger_press(timeout, message, progress_callback):
    ...
    while (time.time() - start_time) < timeout:
        ...
        if response and len(response) > 9:
            code = response[9]
            # 0x00 = imagen tomada (dedo presente)
            if code == 0x00:
                print("¡Huella detectada!, Levanta el dedo")
                if progress_callback:
                    progress_callback("¡Huella detectada!, Levanta el dedo")
                # pequeño retardo para estabilidad (no demasiado largo)
                time.sleep(0.5)
                ...
                # código inesperado -> mostrar para debug y seguir probando
                print(f"DEBUG wait_for_finger_press: codigo get_image = {hex(code)}")
                time.sleep(PAUSA_CORTA)
        else:
            # respuesta inválida o None -> esperar y reintentar
            time.sleep(PAUSA_CORTA)

    if progress_callback:
        progress_callback("Tiempo de espera agotado")
    print("Tiempo de espera agotado.")
    return False, None


def wait_for_finger_release(timeout, message, progress_callback):
    ...
    while (time.time() - start_time) < timeout:
        ...
        if response and len(response) > 9:
            code = response[9]
            # Si el sensor NO responde 0x00 (imagen tomada), asumimos que el dedo fue levantado.
            if code != 0x00:
                if progress_callback:
                    progress_callback("Dedo levantado")
                return True
            else:
                # aun hay dedo presente
                time.sleep(PAUSA_CORTA)
        else:
            time.sleep(PAUSA_CORTA)

    if progress_callback:
        progress_callback("Tiempo de espera agotado. El dedo no fue levantado.")
    print("Tiempo de espera agotado. El dedo no fue levantado.")
    return False

def agregar_huella(progress_callback=None):
    if progress_callback:
        progress_callback("=== AGREGAR NUEVA HUELLA ===")
    print("=== AGREGAR NUEVA HUELLA ===")
    posicion = obtener_siguiente_posicion()
    print(f"Usando posición: {posicion}")

    success, resp1 = wait_for_finger_press(TIMEOUT_SEGUNDOS, "Coloque el dedo", progress_callback)
    if not success:
        return False
    ...
    time.sleep(PAUSA_CORTA)
    if not (response and len(response) > 9 and response[9] == 0x00):
        print("Error en IMAGE2TZ (buffer 1). Respuesta:", response)
        return False

    # if not wait_for_finger_release(TIMEOUT_SEGUNDOS, 'Levante el dedo', progress_callback):
    #   return False

    success, resp2 = wait_for_finger_press(TIMEOUT_SEGUNDOS, "Coloque el dedo nuevamente", progress_callback)
    if not success:
        return False
    ...
    time.sleep(PAUSA_CORTA)
    if not (response and len(response) > 9 and response[9] == 0x00):
        print("Error en IMAGE2TZ (buffer 2). Respuesta:", response)
        return False
...
def detectar_huella(progress_callback=None):
    progress_callback("=== DETECTAR HUELLA ===")
    print("=== DETECTAR HUELLA ===")
    # send_data("display", {"mensaje":"Esperando Huella"})
    
    success, _ = wait_for_finger_press(TIMEOUT_SEGUNDOS, "Esperando huella",progress_callback)
    if not success:
        return None
